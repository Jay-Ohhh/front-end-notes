...
除了可以在声明文件中通过 `import` 导入另一个声明文件中的类型之外，还有一个语法也可以用来导入另一个声明文件，那就是三斜线指令。`import`与三斜线指令的区别是：`import`是引入变量（但不能在全局变量的声明文件最外层使用）， 三斜线指令是引入声明文件。
...
```json
{
  "compilerOptions": {
    ...
    "types": [],                           /* types用来指定需要包含的模块，只有在这里列出的模块的声明文件才会被加载进来，即不用import，可以直接在ts文件中使用 */
