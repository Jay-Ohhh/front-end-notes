...
- eslint-config-prettier：解决ESLint中的样式规范和prettier中样式规范的冲突，以prettier的样式规范为准，使ESLint中的样式规范自动失效，eslint-plugin-prettier已覆盖，无需安装

  > https://github.com/prettier/eslint-config-prettier/blob/6537dd961a7b612d6104017234024f241c2b7591/CHANGELOG.md
  >
...
eslint插件会优先执行`.eslintrc.js`文件的配置。
...
```json
"scripts":{
	"lint": "cross-env NODE_ENV=production eslint './src/**' --fix --ext .js --ext .jsx --ext .ts --ext .tsx",
},
"husky": {
   "hooks": {
      "pre-commit": "npm run lint-staged"
